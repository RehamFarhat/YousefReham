...
class InTwoPhaseLSTM(nn.Module):

    """
    Represents a two-phase LSTM model that incorporates additional features.
    """
    ...
    def __init__(self, weights_matrix, h_dim, n_layers=1, dropout=0.1):
        ...
        self.stance = PeStanceLSTM(weights_matrix, h_dim, n_layers, dropout)
        self.pe = PositionalEncoder(h_dim)
        self.linear1 = nn.Linear(h_dim + 2, 3)
        self.log_softmax = nn.LogSoftmax(dim=1)
        self.linear2 = nn.Linear(h_dim + 2, 3)


    def forward(self, x_src, x_long, x_lat, **kw):
        S, B = x_src.shape

        # context is (L, B, H)
        h1, context = self.sub(x_src, **kw)
        stance_outputs = []
        stance_input = x_src[[0], :]
        h = self.pe(h1)
        for t in range(1, S):
        ...
        stance_out = torch.cat(stance_outputs, dim=0)
        stance_out = torch.mean(stance_out, 0)
        x_long = torch.unsqueeze(x_long,0)
        x_lat = torch.unsqueeze(x_lat,0)
#         x_trg = torch.unsqueeze(x_trg,0)
        stance_out = torch.cat((stance_out.t(), x_long, x_lat))
        out = self.linear1(stance_out.t())
        y_stance = self.log_softmax(out)

        h1 = torch.mean(h1, 0)
        out = torch.cat((h1.t(), x_long, x_lat))
        sub_output = self.linear2(out.t())
        y_sub = self.log_softmax(sub_output)
        # Output shape: (B, 3)
        return y_stance, y_sub
...
# data_loader: the data to classify
def classifier(model, data_loader):
    ...
    for idx_batch, batch in enumerate(data_loader):
            ...
            with torch.no_grad():
                ...
                y_stance_hat, y_sub_hat = model(X,X_long,X_lat)
            ...
            if y_stance_pred.item() ==0:
                opposed.append(idx_batch)
                continue

            if y_stance_pred.item() ==1:
                supporter.append(idx_batch)
            else:
                neutral.append(idx_batch)

    return opposed,supporter,  neutral
