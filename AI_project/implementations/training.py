...
def train_in_two_phase_rnn(model, dl_train, optimizer, sub_loss_fn, stance_loss_fn, max_batches=None):
    losses = []

    with tqdm.tqdm(total=(max_batches if max_batches else len(dl_train)), file=sys.stdout) as pbar:
        for idx_batch, batch in enumerate(dl_train, start=1):
            x, x_long, x_lat, x_trg, y_stance, y_sub = batch.TWEET, batch.LONG, batch.LAT, batch.TARGET, batch.STANCE, batch.SENTIMENT
...
def eval_in_two_phase_rnn(model, dl_test):
    sub_accuracies = []
    stance_accuracies = []
